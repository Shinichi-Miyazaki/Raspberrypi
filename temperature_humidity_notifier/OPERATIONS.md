...
**この手順書の前提**: Piが起動していて、SSHで接続できる状態であること。
Piが新品の場合、microSDカードを入れ直す場合、Piを増設・交換する場合は、
**先に [SETUP_PI.md](SETUP_PI.md)（Raspberry Pi 初期構築手順書）を実施してから**
このフェーズ1に進んでください。すでに動いているPiを引き継いだ場合は不要です。
...
2. **範囲逸脱アラート**: どこかの部屋の温度・湿度が決めた許容範囲を2回続けて外れたとき
   （インキュベータの開けっ放しなどを想定）
...
**⚠ 誤解しやすい点**: この設定を入れると、`mount | grep sensor_data` を実行しても
**何も出ないことがあります。これは正常です**（5分間アクセスが無くて切り離された状態）。
故障と間違えないでください。確認したいときは、先に `ls /mnt/sensor_data` を実行して
中身を表示させてから `mount | grep sensor_data` を見ます。正常なら次の2行が出ます:

```
systemd-1 on /mnt/sensor_data type autofs (...)          ← 受け口
//【NASのIP】/sensor_data on /mnt/sensor_data type cifs (...)  ← 実際の接続
```

常に見ておきたい場合は `systemctl status mnt-sensor_data.automount` を使います。
`Active: active (running)` または `active (waiting)` なら受け口が生きています。
...
許容範囲を一時的にわざと狭くして、本当に通知が来るか試します。
```
$ nano /mnt/sensor_data/config/thresholds.yaml
```
テストしたい部屋（例: 246）の `temp_range` を、今の室温を確実に外す狭い範囲
（例: `temp_range: [0, 1]`）に書き換えて保存する
→ 範囲逸脱アラートは「2回連続で範囲外」で鳴るので、整理（ingest）を少し間をあけて2回動かす:
```
$ sudo systemctl start collector-ingest
（10分ほど待って、新しい測定が1つ増えてから）
$ sudo systemctl start collector-ingest
```
→ Slackに範囲逸脱アラート（「温度が低すぎます」等）が届くことを確認
→ **必ず `temp_range` を元の値に戻して保存する（戻し忘れると鳴りっぱなしになる）**

**5-4. 運用開始**

範囲逸脱アラートは、許容範囲を設定した直後から有効になります（学習のための待ち時間はありません）。
各部屋の `temp_range` / `humid_range` が実態に合った値になっているか、
「アラートの感度調整（許容範囲の変更）」を参考に確認してください。
運用開始日をここに記録: ____年____月____日

### フェーズ5完了条件
- [ ] 4デバイス分のデータがNASに届き、取り込まれている
- [ ] 手動実行で週次グラフがSlackに届いた
- [ ] アラートテストをした場合、temp_range を元の値に戻した
...
- **範囲逸脱アラート**（温度・湿度が高すぎ／低すぎ、という通知）: その部屋に行って、次を確認する
  - **インキュベータ（培養器）や冷蔵庫などのドアが開けっぱなしになっていないか**（最も多い原因）
  - エアコンや温調機が止まっていないか・設定が変わっていないか
  - 確認して問題なければ様子見でOK。設備の故障なら管理者へ連絡
  - 「決めた範囲が実態と合っていない（頻繁に鳴る／鳴るべき時に鳴らない）」場合は、
    このあとの「アラートの感度調整（許容範囲の変更）」で範囲を見直す
...
$ nano /mnt/sensor_data/config/thresholds.yaml
```
該当する部屋の `temp_range` / `humid_range` の数字を `[下限, 上限]` の形で変える:

- 誤報が多い（開けっ放しでもないのに鳴る）→ 範囲を**広げる**。例: `[20, 28]` → `[18, 30]`
- 見逃しがある（逸脱していたのに鳴らなかった）→ 範囲を**狭める**。例: `[20, 28]` → `[21, 27]`
- 湿度も監視したい → その部屋に `humid_range: [30, 70]` の行を追加する
- ある項目を監視したくない → その行を消す（またはコメントアウト）と、その項目は判定されない

範囲逸脱アラートは「同じ側に2回連続で範囲外」のときだけ鳴るので、一瞬のドア開閉では鳴りません。
変更したら「いつ・どの部屋・どの範囲をどう変えたか」をメモに残すこと。
...
（`ls` を先に実行するのは、5分アイドルで切り離されていると `mount` に何も出ないためです）:
```
$ ls /mnt/sensor_data
...
| 許容範囲（temp_range / humid_range） | 各部屋で「この範囲なら正常」とみなす温度・湿度の上下限。`[下限, 上限]` で設定し、同じ側に2回連続で外れると範囲逸脱アラートが鳴る |
